...
import { getHonorOfKingsPackageById } from "@/data/honorOfKingsPackages";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { toast } from "@/components/ui/use-toast";

interface HonorOfKingsCheckoutPageProps {
  onLogout: () => void;
}
...
const HonorOfKingsCheckoutPage = ({ onLogout }: HonorOfKingsCheckoutPageProps) => {
  ...
  useEffect(() => {
    ...
    const storedPlayerID = localStorage.getItem("honorPlayerID");
    const storedUsername = localStorage.getItem("honorUsername");
    
    if (!storedPlayerID) {
      navigate(`/honor-of-kings/purchase/${id}`);
      return;
    }
  ...
  const handleCompletePurchase = () => {
    ...
    setTimeout(() => {
      ...
      if (honorPackage) {
        localStorage.setItem("purchaseAmount", honorPackage.price.toString());
        localStorage.setItem("tokenAmount", (honorPackage.baseAmount + (honorPackage.bonusAmount || 0)).toString());
        localStorage.setItem("playerId", playerID);
        localStorage.setItem("playerName", username || "Player");
        localStorage.setItem("paymentMethod", selectedPayment === "card" ? "Credit Card" : "PayPal");
        localStorage.setItem("gameType", "Honor of Kings");
        localStorage.setItem("currencyType", "Tokens");
        
        localStorage.setItem("purchaseDetails", JSON.stringify({
          packageId: honorPackage.id,
          baseAmount: honorPackage.baseAmount,
          bonusAmount: honorPackage.bonusAmount || 0,
          price: honorPackage.price,
          playerID,
          username,
          paymentMethod: selectedPayment,
          gameType: "Honor of Kings",
          currencyType: "Tokens",
          transactionId: "HOK" + Math.floor(Math.random() * 1000000000),
            ...
            <div className="lg:col-span-1 order-1 lg:order-2">
              ...
              >
                ...
                <div className="flex items-center mb-6 pb-4 border-b border-gray-700">
                  <img src={honorPackage.image} alt="Honor of Kings Tokens" className="w-[70px] mr-4" />
                  ...
                  <div>
                    ...
                    <div className="mt-1">
                      <span className="text-midasbuy-gold font-medium">{honorPackage.price.toFixed(2)} USD</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3 mb-6">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-white">{honorPackage.price.toFixed(2)} USD</span>
                  </div>
                  ...
                  {honorPackage.originalPrice > honorPackage.price && (
                    <div className="flex justify-between text-sm">
                      <span className="text-midasbuy-gold">Discount</span>
                      <span className="text-midasbuy-gold">
                        -{(honorPackage.originalPrice - honorPackage.price).toFixed(2)} USD
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="flex justify-between mb-6 pb-2 border-b border-gray-700">
                  ...
                  <span className="font-bold text-white text-xl">{honorPackage.price.toFixed(2)} USD</span>
                </div>
...
export default HonorOfKingsCheckoutPage;
